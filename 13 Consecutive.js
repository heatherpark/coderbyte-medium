...
from the lowest number to the highest number. For example: If arr contains [4, 8, 6]
then the output should be 2 because two numbers need to be added to the array (5 and 7)
...
function Consecutive(arr) {
  ...
  // loop through every number from arr's min to arr's max.
  for (var i = arr[0]; i < arr[arr.length - 1]; i++) {
    // if number is not found in arr, increment counter.
    if (arr.indexOf(i) === -1) {
      counter++;
    }
  }
